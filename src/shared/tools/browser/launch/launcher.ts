...
import { BrowserError } from "@/shared/utils/errors.js";
import { logger, sleep } from "@/shared/utils.js";
import { BrowserConfig, BrowserDelays } from "../constants.js";
import type {
  ChromeLaunchOptions,
  ChromeSession,
  ChromeVersionResponse,
} from "../types/session.js";
import { bindBrowser } from "../lifecycle/shutdown.js";
import { openPage } from "../actions/elements.js";
...
async function fetchWebSocketDebuggerUrl(
  ...
  for (let attempt = 1; attempt <= BrowserConfig.WS_FETCH_RETRIES; attempt += 1) {
    ...
    await sleep({
      ms: BrowserDelays.WS_FETCH_INTERVAL,
...
export async function launchChromeSession(
  ...
  await sleep({
    ms: BrowserDelays.POST_KILL,
    reason: "debug-profile Chrome processes to exit",
  });
